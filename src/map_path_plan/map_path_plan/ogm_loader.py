...
def clamp_point_on_ray(F: np.ndarray,        # 수선의 발(월드)
                       ...
                       *,
                       ...
                       voxel_size: float,
                       map_shape: tuple[int, int],
                       border_width: int = 10) -> np.ndarray:
    """
    F 가 맵 밖이면, O→v 방향 선분을 지도 경계(테두리 제외)와 만나는 지점으로 잘라
    ...
    # 테두리 제외한 내부 경계 계산
    x_min = origin[0] + border_width * voxel_size
    x_max = origin[0] + (map_shape[1] - 1 - border_width) * voxel_size
    z_min = origin[1] + border_width * voxel_size
    z_max = origin[1] + (map_shape[0] - 1 - border_width) * voxel_size
...
class OGMAndPathLoader(Node):
    ...
    def update_dynamic(self):
        ...
        self.get_logger().info(f"fffffffffffffffffffffff : {sx}, {sz}")

        front_flag = is_front(sx, sz, ex, ez, h_g) # 내전차 우 전방, 적 전차 우 전방 헤딩
        if self.need_replan == True:
            if front_flag:
                ...
                F_left_cl  = clamp_point_on_ray(
                    ...
                    origin=origin, voxel_size=voxel_size, map_shape=(nx, nz), border_width=15)
